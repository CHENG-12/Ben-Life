var index = 0 ;//全局变量记录当前哪一张是激活
var interval = 3000; //轮播间隔时间
var timer = null;//保存计时器对象
var count = 4;//总共实际要轮播的图片数量
var lock = false; //标识是否在执行动画
//自动播放的时候
//点击轮播图指示器的时候
//点击上一张下一张按钮的时候
function slide(nextIndex){
	if(lock) return;//如果当前正在执行轮播动画则直接返回
	lock = true;//进去后赶紧加锁,表示某一个动画开始了,下面开始执行轮播动画
	// console.log(nextIndex);
	// nextIndex: -1 0 1 2 3 4
	//index: 0 1 2 3
	//图片自动切换
	var slider = document.querySelector("ul.banner-slider");
	slider.style.transitionDuration = "1s";
	slider.style.marginLeft = -1 * nextIndex + "00%";
	//指示器切换
	var indicators = document.querySelectorAll("ul.banner-indicator>li");
	indicators[index].className = '';
	
	//更新index
	if(nextIndex === count ) index = 0;
	else if(nextIndex === -1) index = count -1 ;
	else index = nextIndex;
	indicators[index].className = 'active';
	//重置实现无缝效果
	setTimeout(function(){
		// lock = false;//动画结束后,开锁
		slider.style.transitionDuration = "0s";
		if(nextIndex === count) slider.style.marginLeft = "0%";
		if(nextIndex === -1) slider.style.marginLeft = -1 * (count - 1) + "00%";
		lock = false;//动画结束后,开锁
	} , 1020);
	
	
}//函数的标准写法,不要加分号
//自动播放
function play(){
	timer = setInterval(function(){
	
		slide(index + 1);
		
	},interval);
}

//鼠标滑入banner区域,停止自动播放
document.querySelector(".banner").onmouseover = function(){clearInterval(timer)};
//鼠标滑出banner区域,回复自动播放
document.querySelector(".banner").onmouseout = function(){play();};
//指示器点击切换轮播图
var indicators = document.querySelectorAll("ul.banner-indicator>li");
for(var i = 0; i <indicators.length ; i++) {
	indicators[i].a = i;//开辟一个属性a,藏自己的编号
	indicators[i].onclick = function(){
		if(this.className === "active") return;//已经是激活的状态就不再去执行63行的代码
		console.log(123);
		slide(this.a);//再拿出来用这个序号
		
	}
	
}
//上一张的点击事件
document.querySelector("span.banner-prev").onclick = function(){
	slide(index - 1);
}
//下一张的点击事件
document.querySelector("span.banner-next").onclick = function(){
	slide(index + 1);
}

play();